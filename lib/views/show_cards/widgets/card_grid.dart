...
class CardGrid extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final cardsAsyncValue = ref.watch(cardsStreamProvider((gameId, name)));
    ...
    return Container(
      ...
      child: cardsAsyncValue.when(
        ...
        data: (cardList) {
          ...
          double aspectRatio = _calculateAspectRatio(cardList.length, width, height);
          ...
          return GridView.count(
            ...
            children: cardList.map((data) => CardItem(data: data)).toList(),
          );
        },
      ),
    );
  ...
  double _calculateAspectRatio(int length, double gridWidth, double gridHeight) {
    const int crossAxisCount = 4;
    const double crossAxisSpacing = 15.0;
    const double mainAxisSpacing = 15.0;

    final double requiredRows = (length / crossAxisCount).ceilToDouble();
